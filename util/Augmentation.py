...
def center_to_corner_numpy(boxes) :
...
def corner_to_center_numpy(boxes):
...
class BBoxToAbsoluteCoords(object):
    """ 将归一化的边界框还原为原始边界框 """
...
class BBoxToPercentCoords(object):
    """ 将未归一化的边界框归一化 """
...
class  SSDAugmentation(object):
    ...
    def __init__(self, size=[416,416], mean=(0.406, 0.456, 0.485), std=(0.225, 0.224, 0.229)) :
        ...
        self.transformers = Compose([
            ConvertFromInts(),#int to float
            ToAbsoluteCoords(),#box to absolute coordination
            PhotometricDistort(),
            #Expand(self.mean),
            #RandomSampleCrop(),
            RandomMirror(),
...
if __name__ == '__main__':
    line = "../../VOCdevkit/VOC2007/JPEGImages/009564.jpg 108,222,217,319,1 122,144,192,275,14 271,164,406,262,6 205,173,285,214,6 70,171,204,218,6".split()
    #print(line[0])
    image = cv2.imread(line[0])
    #image = cv2.cvtColor(cv2.imread("../../VOCdevkit/VOC2007/JPEGImages/009564.jpg"), cv2.COLOR_BGR2RGB)
    ...
    for box in boxes:
        cx, cy, w, h= box
        xmin=image2.shape[1]*(cx-w/2)
        ymin=image2.shape[0]*(cy-h/2)
        xmax=image2.shape[1]*(cx+w/2)
        ymax=image2.shape[0]*(cy+h/2)
        image2 = cv2.rectangle(image2, (int(xmin), int(ymin)), (int(xmax), int(ymax)), (0, 0, 255), 2)
